...
void initTft()
{
	...
	tft.println("TIME: 23.06.1993 07:00");
}
...
void handleRadioMsg(long nowUnix)
{
	...
	if (command.startsWith("[nextWatering] "))
	{
		...
		command.replace("[time] ", "");
		long currentTime = command.toInt();
...
void updateClockAndWateringTime(RTCDateTime now, long nextWateringTime)
{
	...
	String timeStr = String(now.year) + String("-") + String(now.month) + String("-") + String(now.day) + String(" ") + String(now.hour)
			+ String(":") + String(now.minute) + String(":") + String(now.second);
